...
module.exports = async (helpers, fields, meta) => {
  helpers.gatsby.reporter.verbose(
    formatMessage(
      `Looking for remote file fields on node`
    )
  )
...
/**
 * Load a remote file and replace the remote file field with a Gatsby FK field linking
 * to the created File node.
 * 
 * @param {*} helpers 
 * @param {Object} object 
...
async function handleRemoteFileKeyInObject(helpers, object, key, meta) {
  ...
  if (isString(value)) {
    gatsby.reporter.verbose(
      formatMessage(`Loading remote file (key: "${key}", url: "${value}")`)
    )
    filenode = await loadRemoteFile(helpers, value, meta.id)
  }
  ...
  if (filenode) {
    ...
    object[referenceKey] = filenode.id
  } else {
    const message = formatMessage(
      `Unable to load remote file, using the original value of the field (key: "${key}", value: ${JSON.stringify(
        value
      )})`
    )
