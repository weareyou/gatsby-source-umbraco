...
async function handleRemoteFileKeyInObject(helpers, object, key, meta) {
  ...
  if (isString(value)) {
    gatsby.reporter.verbose(formatMessage(`Handeling remote file field (key: "${key}")`))
    ...
    gatsby.reporter.verbose(formatMessage(`Handeling remote file array (key: "${key}")`))
    const filenodes = await Promise.all(value.map(async url => await loadRemoteFile(helpers, url, meta.id)))
...
function panicOnInvalidRemoteFileType({ gatsby }, key, value) {
  ...
  const message = formatMessage(
    `Encountered invalid type in remote file ${isArray(value) ? `array` : `field`}:`,
    ``,
    `key: ${key}`,
...
async function loadRemoteFile({ gatsby }, url, parentNodeId) {
  ...
  try {
    ...
    const message = formatMessage(`Problem creating remote file node (url: "${url}")`)
    gatsby.reporter.panic(message)
  }
