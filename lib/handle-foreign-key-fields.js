...
function handleForeignKeyFieldsOnObject(helpers, object) {
  ...
  for (const [key, value] of entries) {
    if (key.endsWith(foreignKeySuffix)) {
      if (isIdType(value)) {
        helpers.gatsby.reporter.verbose(formatMessage(`Handling foreign key field (key: "${key}", value: "${value}")`))
        ...
        helpers.gatsby.reporter.verbose(formatMessage(`Handling foreign key array (key: "${key}", value: ${JSON.stringify(value)})`))
        ...
        helpers.gatsby.reporter.panic(message)
      }
    }
