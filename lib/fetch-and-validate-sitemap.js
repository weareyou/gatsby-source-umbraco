...
module.exports = async (helpers) => {
  const route = "/sitemap" // TODO make sitemap route configurable
  const reporter = helpers.gatsby.reporter

  reporter.verbose(formatMessage(`Fetching sitemap from: ${route}`))
  let sitemap = await fetchSitemap(helpers, route)
  reporter.verbose(formatMessage(`Validating sitemap`))
...
async function fetchSitemap({ gatsby, axios }, route) {
  try {
    ...
    const message = formatMessage(`Problem loading sitemap from: ${route}.`)
