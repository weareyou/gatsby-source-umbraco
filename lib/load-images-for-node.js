...
module.exports = async (helpers, fields, meta) => {
  helpers.gatsby.reporter.verbose(
    formatMessage(
      `Looking for images on node (name: "${fields.name}", slug: "${fields.slug}")`
    )
  )
...
async function loadImagesForObject(helpers, fields, meta) {
  ...
  await asyncForEach(entries, async ([key, value]) => {
    if (key.endsWith(remoteFileSuffix)) {
      await handleImageKeyInObject(helpers, object, key, meta)
    } else if (isObject(value)) {
      object[key] = await loadImagesForObject(helpers, value, meta)
    } else if (isArray(value))
      object[key] = await loadImagesForArray(helpers, value, meta)
  })
...
async function handleImageKeyInObject(helpers, fields, key, meta) {
  ...
  if (isString(value)) {
    gatsby.reporter.verbose(
      formatMessage(`Loading image (key: "${key}", url: "${value}")`)
    )
    filenode = await loadImage(helpers, value, meta.id)
  }

  if (filenode) {
    // Replace field with link to new file node
    delete fields[key]
    const referenceKey = getGatsbyReferenceKey(key, options.remoteFileSuffix)
    fields[referenceKey] = filenode.id
  } else {
    const message = formatMessage(
      `Unable to load image, using the original value of the field (key: "${key}", value: ${JSON.stringify(
        value
      )})`
    )
    gatsby.reporter.warn(message)
  }
}
...
async function loadImagesForArray(helpers, array, meta) {
  ...
  await asyncForEach(array, async value => {
    let newValue = value
    if (isObject(value))
      newValue = await loadImagesForObject(helpers, value, meta)
